...
public class ChatServiceImpl implements ChatService {
    ...
    public Flux<String> streamChat(String sessionId, String question) {
        //获取之前的人机交互的聊天记录
        Set<ChatMessageItem> chatHistory = redisTemplate.opsForSet().members(TECH_COMMUNITY_AI_CHAT_SESSION_MESSAGES);
        List<Message> chatHistoryList = chatHistory.stream().map(history -> history.getRole().equals(ChatRole.USER)
                ? new UserMessage(history.getContent())
                : new AssistantMessage(history.getContent())).collect(Collectors.toList());
        //利用授予角色和上传聊天记录来改造用户问题
        ...
        ChatMessageItem userMessageItem = ChatMessageItem.builder()
                ...
                .build();
        redisTemplate.opsForSet().add(TECH_COMMUNITY_AI_CHAT_SESSION_MESSAGES + sessionId, userMessageItem);
        //组装prompt
        ...
        return aiAskStream.doOnNext(s -> askContentBuilder.append(s))
                .doOnComplete(() -> {
                    ChatMessageItem assistantMessageItem = ChatMessageItem.builder()
                            ...
                            .createdTime(LocalDateTime.now())
                            .build();
                    redisTemplate.opsForSet().add(TECH_COMMUNITY_AI_CHAT_SESSION_MESSAGES + sessionId, assistantMessageItem);
                });
    }
}